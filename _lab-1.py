# -*- coding: utf-8 -*-
"""
Created on Wed Oct  4 19:11:17 2023
...
max_x = len(maze[0]) # ширина лабиринта
max_y = len(maze) # высота лабиринта
    
POSSIBLE_WAYS = ['U', 'D', 'R', 'L'] # все возможные способы передвижения
coord_treasure = [0, 0] # координаты сокровища


# Все, что касается движения кладоискателя

def step(coord, direction): # шаг
    if direction == 'U':
...
def step_up(coord): # шаг вверх
    return [coord[0], coord[1]-1]
def step_right(coord): # шаг направо
    return [coord[0]+1, coord[1]]
def step_down(coord): # шаг вниз
    return [coord[0], coord[1]+1]
def step_left(coord): # шаг влево
    return [coord[0]-1, coord[1]]

def cut_way_back(direction): # защита от возврата назад
    if direction == 'U':
        return ('U', 'R', 'L')
    if direction == 'D':
        return ('D', 'R', 'L')
...
# все, что касается изменения файла и проверки
...
# Все, что касается поиска пути и его построения
...
def find_a_way(maze, coord, possible_ways):
    ...
    if len(current_path) > len(path_to_exit): # убираем длинные пути
        return
